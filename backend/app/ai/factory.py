...
def create_ai_provider(settings: Settings) -> AIProvider | None:
    ...
    if not (settings.ai_command_interpretation_enabled or settings.ai_narration_enabled or settings.ai_npc_enabled or settings.ai_world_generation_enabled):
        return None
    if settings.ai_provider == "fake":
        if settings.app_env.casefold() == "production":
            raise UnsupportedAIProviderError(
                "FakeAIProvider is not allowed in production when AI features are enabled."
            )
        return FakeAIProvider()
    if settings.ai_provider == "openai":
        if settings.app_env.casefold() != "development":
            raise UnsupportedAIProviderError("OpenAI features are development-only.")
    ...
    raise UnsupportedAIProviderError(
        f"AI provider {settings.ai_provider!r} is not implemented."
    )
